# this server App created by ASE_Libnt all credit back to the orignal writter >>Ahmed<<
## and this project is Open-sourced so any one can modify and use it for his one need ....
### hope everyone get the best out of this project >>>.
    ...
    if request.method == "GET":
        ...
        if len(users) > 0:
            if users[0]["username"] == username and sha256_crypt.verify(password, users[0]["password"]) == True:
                ...
                return render_template("login.html", error="اسم المستخدم او الرقم السري غير صحيح ")
        else:
            return render_template("login.html", error="اسم المستخدم او الرقم السري غير صحيح")
        ...
        if request.method == "POST":
            ...
            for i in range(len(admins)):
                if admins[i]["username"] == username and admins[i]["id"] != int(id):
                    return jsonify([{"status": "error", "msg": "هذا الاسم غير موجود."}])
            ...
            if image:
                if allowed_file(image.filename) == True:
                    ...
                    return jsonify([{"status": "error", "msg": "نوع الملف غير مدعوم"}])
            ...
            return jsonify([{"status": "success", "msg": "تم حفظ التغييرات"}])
    else:
        return redirect(url_for("home"))
    ...
    if g.user and g.role == "root":
        if request.method == "POST":
            ...
            if firstname == "" or lastname == "" or username == "" or password == "":
                return jsonify([{"status": "error", "msg": "معلومات غير مكتملة"}])

            for i in range(len(admins)):
                if admins[i]["username"] == username:
                    return jsonify([{"status": "error", "msg": "اسم المستخدم غير موجود"}])
        ...
        if request.method == "POST":
            ...
            for i in range(len(admins)):
                if admins[i]["id"] == int(id):
                    ...
                    return jsonify([{"status": "success", "msg": "حذف", "firstname": firstname, "lastname": lastname}])
    else:
        return redirect(url_for("home"))
        ...
        if request.method == "POST":
            ...
            if password != "":
                if password != confirmpassword:
                    return jsonify([{"status": "error", "msg": "تأكيد الرقم السري"}])

                if sha256_crypt.verify(oldpassword, users[0]["password"]) == True:
                    db.execute("UPDATE admins SET password=:password WHERE id=:id", password=sha256_crypt.hash(password), id=int(id))
                else:
                    return jsonify([{"status": "error", "msg": "الرقم السري غير صحيح"}])
            ...
            if image:
                if allowed_file(image.filename) == True:
                    ...
                    return jsonify([{"status": "error", "msg": "الملف غير مدعوم"}])
...
def studentprofile(id):
    if g.user:
        ...
        if len(student) > 0:
            ...
            return render_template("notfound.html", msg="الطالب غير موجود")
    else:
        return redirect(url_for("home"))
    ...
    if g.user:
        if request.method == "POST":
            ...
            if firstname == "" or lastname == "" or fathername == "" or contact == "" or gender == "" or dob == "" or address == "" or class_ == "" or admissiondate == "" or monthlyfee == "" or status == "":
                return jsonify([{"status": "error", "msg": "المعلومات غير مكتملة"}])

            if len(student) < 1:
                return jsonify([{"status": "error", "msg": "الطالب غير موجود"}])

            if status != "Active" and status != "Inactive":
                return jsonify([{"status": "error", "msg": "معلومات غير صحيحة"}])
...
def fetchtestrecord(id):
    if g.user:
        ...
        if len(records) < 1:
            return render_template("notfound.html", msg="السجل غير موجود")
        else:
            records.reverse()
...
def addnewtestrecord():
    if g.user:
        if request.method == "POST":
            ...
            if len(student) < 1:
                return jsonify([{"status": "error", "msg": "لايوجد طالب بهذا الرقم"}])
...
def addnewfeerecord():
    if g.user:
        if request.method == "POST":
            ...
            return jsonify([{"status": "success", "msg": "تم حفظ التغييرات", "lastrowID": lastR_ID}])
    else:
        return redirect(url_for("home"))
### Edit fee record
...
def savesystemsettings():
    if g.user and g.role == "root":
        if request.method == "POST":
            ...
            if institutionname == "":
                return jsonify([{"status": "error", "msg": "اسم المؤسسة الزامي"}])
            
            if (nameinheader != "true" and nameinheader != "false") or (logoinheader != "true" and logoinheader != "false"):
                return jsonify([{"status": "error", "msg": "خطأ فى الادخال "}])
